...
void OS_PeriodTrigger1_Init(int32_t *semaPt, uint32_t period){
  ...
  Period1 = period;
  BSP_PeriodicTask_Init(&RealTimeEvents,10000,0);
}

//****edge-triggered event************
int32_t *edgeSemaphore;
// ******** OS_EdgeTrigger_Init ************
// Initialize button1 and 2, PE0 and 1, to signal on a falling edge interrupt
// Inputs:  semaphore to signal
//          priority
// Outputs: none
...
// 2) no need to unlock PD6
	GPIO_PORTE_DIR_R &= ~0x03;    // (c) make PE1,0 input
  ...
  GPIO_PORTE_PCTL_R &= ~0x000000FF; //  configure PE1,0 as GPIO
	GPIO_PORTE_PUR_R |= 0x03;     //     enable weak pull-up on PE1,0
  GPIO_PORTE_IS_R &= ~0x03;     // (d) PE1,0 is edge-sensitive
  GPIO_PORTE_IBE_R &= ~0x03;    //     PE1,0 is not both edges
	GPIO_PORTE_IEV_R &= ~0x03;    //     PE1,0 is falling edge event
  GPIO_PORTE_ICR_R = 0x03;      // (e) clear flags 1,0
  GPIO_PORTE_IM_R |= 0x03;      // (f) arm interrupt on PE1,0
  NVIC_PRI1_R = (NVIC_PRI1_R&0xFFFFFF00)|(priority << 5); // (g) bits:7-5 for PORTE, set priority to 5
